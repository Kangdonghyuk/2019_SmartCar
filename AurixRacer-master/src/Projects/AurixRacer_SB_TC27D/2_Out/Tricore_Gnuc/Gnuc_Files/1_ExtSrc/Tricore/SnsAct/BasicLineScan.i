...
void BasicLineScan_init(void)
{
...

 {
  ...
  adcChannelConfig[chnIx].resultRegister = (IfxVadc_ChannelResult)(2);
...
cam_information cam_info[3][5];
int debugLine[128], speedLimitLine[3][128];
int nowIndex;
...
int IsNoise(cam_information _cam_info[5]) {
 ...
 for(i=0; i<5; i++)
  noise += ((_cam_info[i].center - _cam_info[(i+1)%5].center < 0) ? -_cam_info[i].center - _cam_info[(i+1)%5].center : _cam_info[i].center - _cam_info[(i+1)%5].center);
...
int GetCameraCenter(int prevServo, int cntDiff) {
 ...
 for(i=0; i<128; i++){
  ...
  cam_info[1][nowIndex].cam_scan[i] = IR_LineScan.adcResult[1][i];
  cam_info[2][nowIndex].cam_scan[i] = IR_LineScan.adcResult[2][i];
 }
 cam_info[0][nowIndex].center = 70;
 ...
 if(isLeftNoise == 1 && isRightNoise == 1)
  ...
  if(((cam_info[1][nowIndex].center - medianLeftCenter < 0) ? -cam_info[1][nowIndex].center - medianLeftCenter : cam_info[1][nowIndex].center - medianLeftCenter) >= 30)
   cam_info[1][nowIndex].center = medianLeftCenter;
  if(((cam_info[2][nowIndex].center - medianRightCenter < 0) ? -cam_info[2][nowIndex].center - medianRightCenter : cam_info[2][nowIndex].center - medianRightCenter) >= 30)
   cam_info[2][nowIndex].center = medianRightCenter;

  if(((medianLeftCenter - medianRightCenter < 0) ? -medianLeftCenter - medianRightCenter : medianLeftCenter - medianRightCenter) >= 60 && ((medianLeftCenter - medianRightCenter < 0) ? -medianLeftCenter - medianRightCenter : medianLeftCenter - medianRightCenter) <= 80) {
   result = medianLeftCenter + medianRightCenter;
   result = result / 2;
  }
  else {
   result = ((70 - medianLeftCenter < 0) ? -70 - medianLeftCenter : 70 - medianLeftCenter) <= ((70 - medianRightCenter < 0) ? -70 - medianRightCenter : 70 - medianRightCenter) ?
     medianLeftCenter + 35 : medianRightCenter - 35;
  }
 }

 result = ((result - 70 < 0) ? -result - 70 : result - 70) > ((cam_info[0][nowIndex].center - 70 < 0) ? -cam_info[0][nowIndex].center - 70 : cam_info[0][nowIndex].center - 70) ? result : cam_info[0][nowIndex].center;
...
int FindCenter(int(*_line)[128]) {
 ...
 for(i=1; i<70; i++) {
  if((70 - i - 1) >= 1) {
   if(leftIndex == 0 && (*_line)[70 - i] != 0 && (*_line)[70 - i - 1] == 0)
    leftIndex = 70 - i;
  }
  if((70 + i + 1) <= 126) {
   if(rightIndex == 127 && (*_line)[70 + i] != 0 && (*_line)[70 + i + 1] == 0)
    rightIndex = 70 + i;
  }
 }
...
int indexWhite = 0;
int FindOneLine(int line[128], int dir) {
...
# 577 "../../MyApp/AurixRacer/0_Src/AppSw/Tricore/SnsAct/BasicLineScan.c"
}
